...
do
	...
	base_dir=/home/jonggyu/frag_test/sqlite_exp/ongoing/HDD/HDD_$ra_size
	mkdir $base_dir
	...
	#	/home/jonggyu/Research/Benchmarks/ACCESS20/disable_gc.sh $dev
		...
		(time /home/jonggyu/Research/Benchmarks/sqlite-bench/sqlite-bench --benchmarks=fillseqsync --use_existing_db=0 --db=/mnt/ --value_size=1024 --num=3000000 --WAL_enabled=0) &> $result_path/sqlite_elapsedTime.txt
		...
		(perf stat grep -r "asdf" /mnt/) &> $result_path/before_read.result

		/home/jonggyu/Scripts/cacheflush.sh

		(perf stat sqlite3 /mnt/dbbench_sqlite3-2.db "SELECT * from test" >/dev/null) &> $result_path/before_select.result
		...
		(perf stat grep -r "asdf" /mnt/) &> $result_path/after_read.result

		/home/jonggyu/Scripts/cacheflush.sh

		(perf stat sqlite3 /mnt/dbbench_sqlite3-2.db "SELECT * from test" >/dev/null) &> $result_path/after_select.result
